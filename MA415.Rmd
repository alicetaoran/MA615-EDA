...
#    b) download and import data files
#    c) normalize data, store final tables
#    d) visualize data, store final graphs
...
```{r Normalize Part I, include = FALSE}
...
t_climate_sum <- aggregate(p_climate$`Frequency (million)`, 
                           by = list(category = p_climate$`Climate Region`),
                           FUN=sum, na.rm = TRUE)

# Update column names
names(t_climate_sum)[1] <- "Climate Region"
names(t_climate_sum)[2] <- "Frequency (million)"

# Merge tables
t_climate_pct <- merge(x = p_climate, y = t_climate_sum, by = "Climate Region", all = TRUE)
names(t_climate_pct)[3] <- "Frequency (million)"
names(t_climate_pct)[4] <- "Frequency in Region (million)"
t_climate_pct$Percentage <- 100*(t_climate_pct$`Frequency (million)` /t_climate_pct$`Frequency in Region (million)`)

# Store results
p_climate <- t_climate_pct
...
```{r Visualize Data}
knitr::opts_chunk$set(fig.width=5.5, fig.height=3.5,cex.lab=0.5, cex.axis=0.5, cex.main=0.5, cex.sub=0.5)
# Total frequency of fridges by unit types
g_unit <-  ggplot(data = p_unit, aes(x = `Unit Type`, y =`Frequency (million)`)) +
...
g_climate <- ggplot(data = p_climate, aes(x = `Climate Region`, 
                                          y = `Frequency (million)`, 
                                          group = `Fridge Type`)) +
             geom_col(aes(fill = `Fridge Type`)) +
             theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
g_climate_pct <- ggplot(data = p_climate, aes(x = `Climate Region`, y = Percentage)) +
                 geom_col(aes(fill = `Fridge Type`)) +
                 labs(title = "Percentage Use Climate Regions", y = "Percentage") +
                 coord_flip()

# Total frequency of fridges by income levels
g_income  <- ggplot(data = p_income, aes(x = Income, y = `Frequency (million)`)) +
             geom_col(aes(fill =`Fridge Type`)) + 
             theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
# Rename columns using convention
colnames(t_fgrth_1)   <- c("f_num", "2015", "2009", "2005", "2001", "1997", "1993")
...
# Update column names before melt
colnames(t_fgrth_3)   <- c("Number of Refrigerators", "2015", "2009", "2005",
                           "2001", "1997", "1993")
...
t_fgrth_all  <- mutate(t_fgrth_all,  `Start Year` = c(`End Year`[-1] + 1, NA))
t_fgrth_one  <- mutate(t_fgrth_one,  `Start Year` = c(`End Year`[-1] + 1, NA))
t_fgrth_more <- mutate(t_fgrth_more, `Start Year` = c(`End Year`[-1] + 1, NA))

# Add a years in period column
t_fgrth_all  <- mutate(t_fgrth_all,  `Years in Period` = `End Year` - `Start Year` + 1)
t_fgrth_one  <- mutate(t_fgrth_one,  `Years in Period` = `End Year` - `Start Year` + 1)
t_fgrth_more <- mutate(t_fgrth_more, `Years in Period` = `End Year` - `Start Year` + 1)
...
t_fgrth_all  <- mutate(t_fgrth_all,
                       `Frequency Change` =  `Frequency (million)` - c(`Frequency (million)`[-1], NA))
...
t_fgrth_all <- mutate(t_fgrth_all,
                      `Percentage Change` = 100 * (`Frequency Change`/`Frequency (million)`[-1]))
...
t_fgrth_all <- mutate(t_fgrth_all,
                      `Yearly Percentage Change` = `Percentage Change` / `Years in Period`)
...
t_fgrth_all_1  <- select(t_fgrth_all,  `Year Period`, `Frequency (million)`,
                         `Frequency Change`, `Percentage Change`, `Yearly Percentage Change`)
t_fgrth_one_1  <- select(t_fgrth_one,  `Year Period`, `Frequency (million)`,
                         `Frequency Change`, `Percentage Change`, `Yearly Percentage Change`)
t_fgrth_more_1 <- select(t_fgrth_more,  `Year Period`, `Frequency (million)`,
                         `Frequency Change`, `Percentage Change`, `Yearly Percentage Change`)
...
kable(p_fgrth_all, caption = 'U.S. Families with Refrigerators 1994-2015 (millions)',
...
kable(p_fgrth_one, caption = 'U.S. Families with one Refrigerator 1994-2015 (millions)',
      format = "latex", booktabs = T) %>%
  kable_styling(font_size = 7, latex_options = c("striped", "hold_position"))
...
kable(p_fgrth_more, caption = 'U.S. Families with two or more Refrigerator 1994-2015 (millions)',
      format = "latex", booktabs = T) %>%
  kable_styling(font_size = 7, latex_options = c("striped", "hold_position"))
```
